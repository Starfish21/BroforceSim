...
class Creature {
  ...
  Creature modified(int id) {
    ...
    if (random(0, 1) < bigMutationChance*mutability || n.size() <= 3) { //Add a node
    ...
    if (random(0, 1) < bigMutationChance*mutability && modifiedCreature.n.size() >= 9) { //Remove a node
      modifiedCreature.removeRandomNode();
    }
    if (random(0, 1) < bigMutationChance*mutability && modifiedCreature.m.size() >= 20) { //Remove a muscle
  ...
  void checkForBadAxons(){
    ...
    for (int i = 0; i < m.size(); i++) {
      Muscle mi = m.get(i);
      if(mi.axon >= m.size()){
        mi.axon = getNewMuscleAxon(m.size());
      }
    }
...
void drawNode(Node ni, float x, float y, int toImage) {
  color c = color(0, 0, 512-int(ni.f*512));
  if (ni.f <= 0.5) {
    c = color(255-int(ni.f*512), 255-int(ni.f*512), 255) ;
  }
  if (toImage == 0) {
    fill(c);
    noStroke();
...
void setGlobalVariables(Creature thisCreature) {
  ...
  simulationTimer = 0;
  energy = baselineEnergy*50*n.size();
  totalNodeNausea = 0;
  averageNodeNausea = 0;
}
